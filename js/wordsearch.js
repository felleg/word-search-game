(function(){
   ...
   WordSearch.prototype.drawmatrix = function() {
      for (var row = 0; row < this.settings.gridSize; row++) {
         ...
         for (var col = 0; col < this.settings.gridSize; col++) {
            ...
            // Add event listeners
            cvEl.addEventListener('click', this.onMouseclick(this.matrix[row][col]));
    ...
    * @param {Object} item
    */
   WordSearch.prototype.onMouseclick = function(item) {
      var _this = this;
      return function() {
         if (_this.selectFrom) {
            ...
            _this.selected = [];
            setTimeout(_this.clearHighlight, 1000);
         } else {
            _this.clearHighlight();
